...
<body>
  ...
  <script>
    ...
    async function refresh(){try{const response=await fetch('/api/classifiers',{cache:'no-store'});if(!response.ok)throw new Error('Status API returned '+response.status);const data=await response.json(),items=data.classifiers;updated.textContent='Updated '+new Date(data.generatedAt).toLocaleTimeString();summary.innerHTML='<span>'+count(items.length)+' classifiers</span><span>'+count(items.filter(x=>x.loaded).length)+' loaded</span><span>'+count(items.filter(x=>x.trained).length)+' trained</span><span>'+count(items.filter(x=>x.training).length)+' training</span>';if(items.length===0){content.innerHTML='<div class="empty">No classifiers have used this data directory yet.</div>';return}content.className='grid';content.innerHTML=items.map(item=>{const measured=item.lastEvaluatedError,within=measured!==null&&measured<=item.acceptableError;const progress=measured===null?0:Math.min(100,item.acceptableError===0?(measured===0?100:0):item.acceptableError/measured*100);return '<article class="card"><div class="card-head"><div><h2>'+esc(item.name)+'</h2><div class="type">'+esc(item.resultType)+' result · '+esc(item.needleVersion||'No model yet')+'</div></div><div class="states"><span class="pill '+(item.loaded?'on':'')+'">'+(item.loaded?'Loaded':'Not loaded')+(item.loadedProcessCount>1?' ×'+item.loadedProcessCount:'')+'</span><span class="pill '+(item.training?'training':'')+'">'+(item.training?'Training':'Idle')+'</span><span class="pill '+(item.trained?'on':'')+'">'+(item.trained?'Trained':'Learning')+'</span></div></div><dl><dt>Collected examples</dt><dd>'+count(item.retainedExamples)+'</dd><dt>Lifetime examples</dt><dd>'+count(item.totalExamplesLogged)+'</dd><dt>Last measured error</dt><dd class="'+(within?'good':measured===null?'':'bad')+'">'+pct(measured)+'</dd><dt>Target error</dt><dd>'+pct(item.acceptableError)+' or less</dd><div class="bar"><i style="width:'+progress+'%"></i></div><dt>Training attempts</dt><dd>'+count(item.trainingAttempts)+'</dd><dt>Local classifications</dt><dd>'+count(item.totalLocalClassifications)+'</dd></dl></article>'}).join('')}catch(error){updated.textContent='Disconnected';content.className='';content.innerHTML='<div class="error">'+esc(error.message||error)+'</div>'}}
